...
def test_greetings_2():
    """Тестирование функции приветствия"""
    time_obj = datetime.datetime.now()
    if 13 <= time_obj.hour <= 18:
        assert greetings() == "Добрый день"

def test_greetings_3():
    """Тестирование функции приветствия"""
    time_obj = datetime.datetime.now()
    if 19 <= time_obj.hour <= 23:
...
def test_currency_rates(mock_get):
    ...
    mock_response_usd.json.return_value = {"conversion_rates": {"RUB": 89.77}}
    mock_response_eur = Mock()
    mock_response_eur.json.return_value = {"conversion_rates": {"RUB": 97.45}}
    mock_get.side_effect = [mock_response_usd, mock_response_eur]

    result = currency_rates(['USD', 'EUR'])
    expected = [{"currency": "USD", "rate": 89.77}, {"currency": "EUR", "rate": 97.45}]
